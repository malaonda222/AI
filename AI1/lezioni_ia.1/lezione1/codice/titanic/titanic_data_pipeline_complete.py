...
class DataPipeline:
    ...
    def load_from_database(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        engine = create_engine(self.config.db_uri) #permette di connettersi all'url del database (sempre con config)
        try:
    ...
    def expand_json_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['preferences'] = df['preferences'].apply(json.loads) #trasformazione dei dati in una colonna, non un’unione
        prefs_expanded = pd.json_normalize(df['preferences'])
        return pd.concat([df.drop('preferences', axis=1), prefs_expanded], axis=1) #rimuove la colonna 'preferences' e la sostituisce con prefs_expanded
