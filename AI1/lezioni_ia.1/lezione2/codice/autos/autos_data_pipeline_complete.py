...
class DataPipeline:
    ...
    def visualize(self, df: pd.DataFrame) -> None:
        ...
        plt.bar(x=df_agg.index.astype(str), height=df['mean_price'], color="blue")
        df_min = df.groupby('make').agg(min_price=('price', 'min'))
        
        df_max = df.groupby('make').agg(max_price=('price','max'))
        df_max_sorted = df_max.sort_values(by='max_price', ascending=True)
        bar_colors_2 = ["tab:red", "tab:blue", "tab:green", "tab:purple"]
        plt.bar(x=df_max_sorted.index.astype(str), height=df_max_sorted['max_price'], color=bar_colors_2)
