...
class DataPipeline:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Sostitutisce NaNs con il valore più frequente 
    ...
    def visualize(self, df: pd.DataFrame) -> None:
        ...
        plt.figure(figsize=(24, 12)) #stabilisce la misura della figura (larghezza e altezza)
        df_agg_1 = df.groupby('preferred_deck').agg(deck_count=('PassengerId', 'count')) #group by: quello ti interessa andare ad analizzare
        df_agg_2 = df.groupby('dining_time').agg(mean_fare=('Fare', 'mean'))             #agg: come lo fai (dati e operazione)
        df_agg_3 = df.groupby('activity').agg(min_age=('Age', 'min'))
        print(type(df_agg_1)) #ti va a stampare il tipo <class 'pandas.core.frame.DataFrame'>
        print(df_agg_1)
        print(df_agg_2)
        print(df_agg_3)
        ...
        plt.subplot(1, 2, 1)
        plt.bar(x=df.index.astype(str), height=df['Fare']) #df.index va a stabilire come index la colonna di sx di default
        plt.title('Tariffa per passeggero')                #se vogliamo impostarla noia allora es: x=df['Age'].astype(str)
        plt.ylabel('Valore')
        ...
        plt.show()
        plt.close() #chiudere una figure di Matplotlib
