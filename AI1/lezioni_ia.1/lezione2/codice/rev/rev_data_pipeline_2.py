...
class DataPipeline:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['Total'] = df.select_dtypes(include=np.number).sum(axis=1) #axis=0 somma verticale, axis=1 somma orizzontale     
        return df                                                     #seleziona in base ai tipi delle colonne e filtra in base a un parametro 

    def visualize_plot(self, df_in: pd.DataFrame) -> None:        
        """Crea e visualizza un Line plot"""
        df = df_in.reset_index(drop=True).copy() #fa una copia del df_in con l'indice resettato e poi imposta Country come indice
        df.index = df['Country']  
    ...
    def visualize_bar(self, df_in: pd.DataFrame) -> None:        
        ...
        plt.subplots(figsize=(10, 6))  #subplot adatto quando si vuole creare un grafico con più sottografici
    ...
    def visualize_pie(self, df_in: pd.DataFrame) -> None:        
        ...
        # colors_list = ['gold', 'yellowgreen', 'lightcoral', 'lightskyblue', 'lightgreen'] # OCCHIO che sia della stessa lunghezza delle regioni  
        # print(len(colors_list))
        
        plt.figure(figsize=(20, 10)) #ti da solo la cornice del grafico, crea solo la figura 
        ...
        if self.config.csv_path.__contains__("cl"):
            ...
            plt.ylabel("")
            plt.axis('equal')
            plt.legend(labels=df_region_95.index, loc='lower left') #si passa parametro labels (regione cioè index) e si definisce la localizzazione della legenda 
        else:
    ...
    def visualize_area(self, df_in: pd.DataFrame) -> None:
        ...
        df_sr = df.groupby('SubRegion')[self.years].sum() #la colonna che viene raggruppata diventa in automatico index
        print(df_sr)
    ...
    def visualize_hist(self, df_in: pd.DataFrame) -> None:      
        ...
        plt.xlabel(xlabel)  
        plt.tight_layout() #sistemare automaticamente gli spazi del grafico (senza questo magari il testo si potrebbe sovrapporre)       
    ...
    def visualize_boxplot(self, df_in: pd.DataFrame) -> None:
        ...
        df.set_index('Country', inplace=True) #imposta Country come indice

        country_name = "India" #cerca India e imposta la variabile 
        country = df.loc[country_name, self.years] #ritorna una series con gli anni come indice e i valori di India per ogni anno 
        print(country)
        country = pd.DataFrame(df.loc[country_name, self.years]) #trasforma in dataframe 
        ...
        new_df = pd.DataFrame({'95-04': df_first_dec, '05-14': df_second_dec, '15-24':df_third_dec}) #creazione di un dataframe con le decadi calcolate 
        print(new_df)
        ...
        ## Opzione 1: prendere l’indice
        # print(df_top.index)
        # # Restituisce un Index con i nomi dei Paesi:
        # # Index(['Italy', 'France', 'Germany', ...], dtype='object', name='Country')
        
        # # Opzione 2: trasformare l’indice in colonna
        # print(df_top.reset_index()[["Country"]])
        # Restituisce un DataFrame con una sola colonna 'Country', contenente i top 15 Paesi
    ...
    def visualize_scatter(self, df_in: pd.DataFrame) -> None:
        ...
        df = df_in.reset_index(drop=True).copy() #crea una copia di df_in e resetta l'index numerico eliminando quello originale 
        
        df_tot = pd.DataFrame(df[self.years].sum(axis=0)) #somma verticale (prende la colonna e somma le righe) dei valori per ogni anno 
        df_tot.index = map(int, df_tot.index) #trasforma ogni indice (year) in intero qualora ci fosse qualche stringa 
        df_tot.reset_index(inplace = True) #resetta index quindi colonna years diventa 'index'
        df_tot.columns = ['year', 'total'] #imposta di nuovo in nomi delle due colonne 
        # print(df_tot)

        # SOLUZIONE 1        
        plt.figure(figsize=(10, 6)) #si prepara il foglio di lavoro

        #opzione 1 con Pandas df.plot(kind='scatter', x='year', y='total', color=' ')
        df_tot.plot(kind='scatter', x='year', y='total', color='darkblue') 

        #opzione 2 plt.scatter( senza x, y, direttamente df['nome_colonna']))
        ...
        #opzione 1 con Pandas con oggetto ax creato 
        ax = df_tot.plot(kind='scatter', x='year', y='total', color='darkblue')

        #opzione 2 ax.scatter(df['year'], df_tot['total], color="")
        ...
        df_tot = add_columns(df_tot) #sovrascrive il df_tot con un altro df_tot arricchito di nuove colonne generate da add_columns
        # print(df_tot) 
        plt.figure(figsize=(10, 6))
        sns.scatterplot(data=df_tot, x="year", y="total", hue="shift", size="employees") #data=indicare il df, x, y, hue=stabilisce il colore, size=dimensione        
        title = "Fatturato totale per anno [1995 - 2024] - cl" if self.config.csv_path.__contains__("cl") else "Fatturato totale per anno [1995 - 2024] - ds"        
        ylabel = "milioni di USD" if self.config.csv_path.__contains__("cl") else "USD"
        plt.title(title) 
        plt.xlabel('Anno')
        ...
        x = df_tot['year']
        y = df_tot['total']   
        coefficients = np.polyfit(x, y, 1) #prende i due parametri x e y e il grado del polinomio, restituisce i coefficienti (slope, intercept)  
        p = np.poly1d(coefficients) #oggetto polinomiale da un array di coefficienti, slope * x + intercept 
        plt.plot(x, p(x), color='red') #x, oggetto polinomiale, colore     
    ...
    #    """Crea e salva visualizzazioni"""
        ...
        plt.subplot(1, 3, 1) #primo grafico a sinistra
        df.index = df['Country']  
        # Andamento fatturato totale sui trent'anni
        rev_tot = df.loc[:, self.years].sum(axis=0)
        rev_tot.plot(kind="line") #plt.plot(rev_tot)   
        title = "Fatturato totale - cl" if self.config.csv_path.__contains__("cl") else "Fatturato totale - ds"
        ylabel = "milioni di USD" if self.config.csv_path.__contains__("cl") else "USD"
        plt.title(title)
        plt.ylabel(ylabel)
        ...
        plt.subplot(1, 3, 2) #grafico centrale a barre verticali, stampa cina e brasile 
        df.index = df['Country']  
        china = df.loc['China', self.years]
        china.plot(kind='bar', color='blue')
        br = df.loc['Brazil', self.years]
        ...
        plt.title('Percentuale fatturato per regione geografica (1995)')
        plt.ylabel("") #obbligatorio per il pie
        plt.axis('equal') #garantisce che farà una torta perfetta 
        plt.legend(labels=df_region_95.index, loc='lower left') 
