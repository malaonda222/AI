...
class DataPipeline:
    ...
    def visualize_area(self, df_in: pd.DataFrame) -> None:
        ...
        df_sr_3_years = df.groupby(['SubRegion']).agg({'1999':'sum', '2000': 'sum', '2001': 'sum'})
        ...
        ax.set_title('Fatturato: primi 5 sottoregioni per importo totale')
        ax.set_ylabel('Milioni di USD')
        ax.set_xlabel('Anni di riferimento 1999 - 2001')
    ...
    def visualize_hist(self, df_in: pd.DataFrame) -> None:      
        ...
        df.index = df['Country']
        df_t = df.loc[['Italy', 'Germany', 'Spain'], self.years].transpose()
        ...
        plt.title('Fatturato di Italia, Germania e Spagna dal 1995 al 2024 - cl')
        plt.ylabel('Numero di Anni')
        plt.xlabel('Fatturato (milioni di USD)')
        plt.show()
        
        paesi = ['Italy', 'Germany', 'Spain']
        df_3_paesi = df[df['Country'].isin(paesi)]
        fig, ax = plt.subplots(figsize=(8, 5))
        bars = ax.hist(df_3_paesi['1995':'2025'], bins=10, color='green', edgecolor='white')
        plt.bar_label(bars[2], fontsize=6, color='navy')
        plt.xticks(bars[1])
        plt.title('Fatturato di Italia, Germania e Spagna dal 1995 al 2024 - ds')
        plt.ylabel('Numero di Anni')
        plt.xlabel('Fatturato (USD)')
        plt.show()
        
        
        # 2) Cambiare paesi, osservare i diversi trend e rifletterci un po' su 
        paesi1 = ['Francia', 'Nigeria', 'Kenya']
        df_3_paesi = df[df['Country'].isin(paesi1)]
        fig, ax = plt.subplots(figsize=(8, 5))
        bars = ax.hist(df_3_paesi['1995':'2025'], bins=10, color='green', edgecolor='white')
        plt.bar_label(bars[2], fontsize=6, color='navy')
        plt.xticks(bars[1])
        plt.title('Fatturato di Francia, Nigeria, Kenya dal 1995 al 2024 - ds')
        plt.ylabel('Numero di Anni')
        plt.xlabel('Fatturato (USD)')
        plt.show()
