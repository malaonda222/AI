...
class DataPipeline:
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # # # ESERCIZI
        # # Aggiungere al dataset una colonna 'Total" contenente il fatturato totale 
        # prodotto da ogni nazione in tutti i 30 anni 
    ...
    def visualize_plot(self, df_in: pd.DataFrame) -> None:        
        ...
        china_japan_kenya = df.loc[['China', 'Japan', 'Kenya'], self.years]
        china_japan_kenya.transpose().plot()
        plt.savefig("../../visual/rev/cjk.png")
        plt.show()
        ...
        china_japan_kenya_decadi = df_decadi.loc[['China', 'Japan', 'Kenya']]
        china_japan_kenya_decadi.transpose().plot()
        plt.title("Andamento fatturato per decadi")
        plt.ylabel("Fatturato")
        plt.xlabel("Totale")
        plt.savefig('../../visual/rev/cjk_decadi')
        plt.show()
        plt.close()
    ...
    def visualize_bar(self, df_in: pd.DataFrame) -> None:        
        ...
        df_fatturato_china.plot(kind='bar', figsize=(20, 9))
        plt.title('Fatturato China')
        plt.xlabel('Anni')
        plt.ylabel('Fatturato')
        plt.savefig("../../visual/rev/fatturato_china")
        plt.show()
    ...
    def visualize_pie(self, df_in: pd.DataFrame) -> None:        
        ...
        plt.figure(figsize=(20, 10))
        df_region_2024 = df.groupby(['Region']).agg({'2024' : 'sum'})
        plt.subplot(1, 2, 1)

        df_region_2024['2024'].plot(kind='pie',
                                    autopct='%1.1f%%',
                                    startangle=90,
                                    labels=None,
                                    pctdistance=1.07,
        ...
        axes[0].set_title("Percentuale fatturato per regione (1995)")
        axes[0].axis("equal")
        axes[0].legend(labels=df_region_95.index, loc='lower right')
        ...
        axes[1].set_title("Percentuale fatturato per regione (2024)")
        axes[1].axis("equal")
        axes[1].legend(labels=df_region_2024.index, loc='lower right')

        plt.tight_layout()
        plt.savefig("../../visual/rev/pie_chart_due")
        plt.show()
